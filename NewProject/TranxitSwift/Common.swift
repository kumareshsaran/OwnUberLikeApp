...
class Common {
    ...
    class func storeUserData(from profile : Profile?){
       ...
       /* User.main.id = profile?.id
        ...
        User.main.wallet_balance = Int(profile?.wallet_balance ?? 0.0)
    ...
    class func setFont(to field :Any, isTitle : Bool = false, size : CGFloat = 0) {
        ...
        let font = UIFont(name: isTitle ? FontCustom.semibold.rawValue : FontCustom.Medium.rawValue, size: customSize)
        switch (field.self) {
        case is UITextField:
    ...
    class func  getChatId(with providerId : Int?) -> String {
        ...
        let chatPath = UserDefaults.standard.value(forKey: "request_id")
        return chatPath as! String
