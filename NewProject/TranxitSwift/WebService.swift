...
class Webservice : PostWebServiceProtocol {
    ...
    func retrieve(api: Base,url : String?, data: Data?, imageData: [String:Data]?, paramters: [String : Any]?, type: HttpType, completion : ((CustomError?, Data?)->())?) {
        ...
        guard reach?.connection == .wifi || reach?.connection == .cellular else {  // Internet not available
            ...
            self.interactor?.on(api: api, error: CustomError(description: "notReachable", code : StatusCode.notreachable.rawValue))
            self.completion?(CustomError(description: "notReachable", code : StatusCode.notreachable.rawValue), nil)
            return
        }
    ...
    fileprivate func send(_ response: (DataResponse<Any>)?) {
        ...
        if response.response?.statusCode == StatusCode.unAuthorized.rawValue  { // Validation For UnAuthorized Login
            ...
            if let error = response.data?.getDecodedObject(from: ErrorLogger.self), error.error != nil { // Retrieving error from Server
                ...
                self.completion?(CustomError(description: "\(error.otp ?? 0)", code: (response.response?.statusCode) ?? StatusCode.ServerError.rawValue), nil)
                if apiType == .signup{
                    if error.otp == nil{
                         self.interactor?.on(api: apiType, error: CustomError(description: "\(error.message ?? "0")", code: (response.response?.statusCode) ?? StatusCode.ServerError.rawValue))
                    }else{
                         self.interactor?.on(api: apiType, error: CustomError(description: "\(error.otp ?? 0)", code: (response.response?.statusCode) ?? StatusCode.ServerError.rawValue))
                    }
...
//             print("User AccessToken:----- \(User.main.accessToken)")
//        }
