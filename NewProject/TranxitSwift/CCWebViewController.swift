...
//    }()
    ...
    private func setupWebView(){
        ...
        _ = [viewWeb.setContentCompressionResistancePriority(UILayoutPriority(1000.0), for: UILayoutConstraintAxis.vertical)]
    }
...
//
    ...
    func webViewDidFinishLoad(_ webView: UIWebView) {
        ...
        if(string.contains("/response")) //("http://122.182.6.216/merchant/ccavResponseHandler.jsp"))//
        {
            print(viewWeb.isLoading)
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        if(string.contains("/response")) //("http://122.182.6.216/merchant/ccavResponseHandler.jsp"))//
        {
            ...
            webView.evaluateJavaScript("document.documentElement.outerHTML") { (data, error) in
                ...
                if error != nil{
                    self.webParsedData = data as? NSString ?? ""
                    return
                }
            }
//            guard let htmlTemp:NSString = webView.evaluateJavaScript(from: "document.documentElement.outerHTML") as NSString? else{
//                print("failed to evaluate javaScript")
//                return
//            }
            
//            let html = htmlTemp
            let html = self.webParsedData
